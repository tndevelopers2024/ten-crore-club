    ...
    <>
      ...
      <article className="px-5 pt-28 pb-20 sm:px-8">
        <div className="mx-auto max-w-3xl">
          <Link
            href="/blog"
            className="inline-flex items-center gap-1.5 text-sm text-gold-light/60 transition-colors hover:text-gold"
          >
                ...
                <ul className="mt-4 space-y-2.5 border-l border-line">
                  {headings.map((h) => (
                    <li key={h.id}>
                      <a
                        href={`#${h.id}`}
                        className="-ml-px block border-l border-transparent pl-4 text-sm text-gold-light/60 transition-colors hover:border-gold hover:text-gold"
                      >
                        {h.text}
                      </a>
                    </li>
              ...
              {post.body.map((block, i) => {
                const node = block.startsWith("## ") ? (
                  <h2
                    ...
                    className="scroll-mt-28 pt-4 font-display text-3xl font-semibold text-gold"
                  >
                    {block.slice(3)}
                  </h2>
                ) : (
                  <p
                    key={i}
                    className="font-display text-xl leading-relaxed text-gold-light/85"
                  >
                    {block}
                  </p>
                );
              ...
              <Reveal as="article" key={r.slug}>
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    <Badge variant="outline">{categoryLabels[r.category]}</Badge>
                    <h3 className="mt-3 font-display text-lg font-semibold leading-snug text-cream group-hover:text-gold">
                      {r.title}
                    </h3>
                    <span className="mt-auto pt-4 text-xs text-gold-light/50">
                      {r.readTime} min read
                    </span>
                  </div>
                </Link>
...
function MidCTA() {
  return (
    <div className="my-4 rounded-lg border border-red-vivid/40 bg-red-deep/10 p-6 text-center">
